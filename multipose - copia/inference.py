...
lstm_model.add(LSTM(128, return_sequences=True, activation='relu', input_shape=(15,36)))
lstm_model.add(LSTM(64, return_sequences=False, activation='relu'))
lstm_model.add(Dense(64, activation='relu'))
lstm_model.add(Dense(32, activation='relu'))
lstm_model.add(Dense(6, activation='softmax'))

lstm_model.load_weights("./version_final_sabado_estable1.h5")
...
actions = np.array(['golpe_derecha_mirada_derecha', 'golpe_izquierda_mirada_derecha','golpe_derecha_mirada_izquierda', 'golpe_izquierda_mirada_izquierda','caminando_derecha','caminando_izquierda'])
...
while cap.isOpened():
    ...
    # Render keypoints 
    loop_through_people(frame, keypoints_with_scores, 0.4)
    
    person1 = keypoints_with_scores[0]
    pose_data = np.array(person1[5:])
    normalized_points=draw_keypoints(frame, pose_data, 0.4)
