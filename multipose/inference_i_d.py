...
#lstm_model.add(Dropout(0.2))  # Regularización con Dropout
lstm_model.add(Dense(2, activation='softmax'))

lstm_model.load_weights("./martes276.h5")
...
actions = np.array(['caminando_derecha','caminando_izquierda'])
...
while cap.isOpened():
    ...
    # Render keypoints 
    loop_through_people(frame, keypoints_with_scores, 0.6)
    
    person1 = keypoints_with_scores[0]
    pose_data = np.array(person1[5:])
    normalized_points=draw_keypoints(frame, pose_data, 0.6)
